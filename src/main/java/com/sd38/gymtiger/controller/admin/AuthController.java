...
import com.sd38.gymtiger.service.AuthService;
import com.sd38.gymtiger.service.CloudinaryService;
...
//        redirectAttributes.addFlashAttribute("x", user);tttttt
    ...
    public String p5(Model model, Principal principal) {
        ...
        model.addAttribute("imgUrl", currentLog.getAvatar());
    ...
    public String updateProfile(@RequestParam("image") MultipartFile file, Principal principal)
            throws IOException {
        try {
            Map uploadResult = cloudinaryService.uploadImage(file);
            String imageUrl = uploadResult.get("url").toString();
            Account currentLog = getCurrentUser(principal);
            var imgUrl = accountService.updateImg((long)currentLog.getId(), imageUrl);
            currentLog.setAvatar(imgUrl);
            return "redirect:/profile";
        } catch (IOException e) {
            throw e;
        }
